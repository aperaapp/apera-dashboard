<template>
  <div class="flex gap-4 w-full p-4">
    <div class="w-[20%]">
      ...
      <ErrorCard v-if="profileError" title="Erorr fetching worker profile" :message="profileError.message" />
      <UCard v-if="profile" :ui="{ body: { padding: 'px-0 sm:p-0' }, rounded: 'rounded-2xl' }">
        <div class="space-y flex flex-col pt-4">
          ...
          <ListItem label="Specialties" :value="profile.specialties.join(', ')" />
          ...
          <ListItem label="Details Submitted">
            <UBadge :label="profile.details_submitted ? 'Yes' : 'No'"
              :color="profile.details_submitted ? 'green' : 'red'" variant="soft"
              :icon="profile.details_submitted ? 'i-heroicons-check-badge' : 'i-heroicons-shield-exclamation'" />
          </ListItem>
          <ListItem label="Payouts Enabled" class="border-b-0">
            <UBadge :label="profile.payouts_enabled ? 'Yes' : 'No'" :color="profile.payouts_enabled ? 'green' : 'red'"
              variant="soft"
              :icon="profile.payouts_enabled ? 'i-heroicons-check-badge' : 'i-heroicons-shield-exclamation'" />
          </ListItem>
        </div>
      </UCard>
    </div>
    <div class="w-[50%]">
      ...
      <ErrorCard v-if="gigsError" title="Error fetching worker profile" :message="gigsError.message" />
    ...
    <div class="w-[30%]">
      ...
      <ErrorCard v-if="payoutsError" title="Error fetching worker profile" :message="payoutsError.message" />

      <UCard :ui="{ body: { padding: 'px-0 sm:p-0' }, rounded: 'rounded-2xl' }">
        <WorkerPayoutsTable v-if="payouts && payouts.length > 0" :payouts="payouts" />
