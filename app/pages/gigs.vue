...
const defaultLocations = gigs.value.reduce((acc, gig: NonNullable<GigViewRow>) => {
    ...
    <UDashboardPanel grow>
      ...
      <UTable v-model:sort="sort" :rows="filterSorted" :columns="columns" sort-mode="manual" class="w-full"
        ...
        <template #created_at-data="{ row }">
          <p class="text-gray-900 dark:text-white font-medium line-clamp-2 max-w-[300px]">{{ new
            Date(row.created_at).toDateString() }}</p>
        </template>
        <template #day-data="{ row }">
          <p class="text-gray-900 dark:text-white font-medium line-clamp-2 max-w-[300px]">{{ new
            Date(row.day).toDateString() }}</p>
        </template>
        <template #required_specialties-data="{ row }">
          <div class="flex items-center gap-3">
            <span class="text-gray-900 dark:text-white font-medium">
              ...
              <span v-if="!row.required_specialties" class="opacity-10"> Empty </span>
              <span v-if="row.required_specialties && row.required_specialties.length > 3" class="ml-2">
                <UKbd>
                  {{ row.required_specialties?.length <= 3 ? "" : `+${row.required_specialties?.slice(2).length} more`
                    }} </UKbd>
              </span>
            </span>
          </div>
