...
const users$1 = eventHandler(async (event) => {
  const { q, statuses, locations, sort, order } = getQuery(event);
  return users.filter((user) => {
    if (!q) return true;
    return user.name.search(new RegExp(q, "i")) !== -1 || user.email.search(new RegExp(q, "i")) !== -1;
  }).filter((user) => {
    if (!(statuses == null ? void 0 : statuses.length)) return true;
    return statuses.includes(user.status);
  }).filter((user) => {
    if (!(locations == null ? void 0 : locations.length)) return true;
