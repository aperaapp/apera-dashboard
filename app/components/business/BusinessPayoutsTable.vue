...
const defaultWorkers = payouts.reduce((acc, payout) => {
...
<template>
  <UDashboardPage>
    <UDashboardPanel grow>
      <UDashboardNavbar title="Payouts" :badge="payouts.length">
        <template #right>
      ...
      <UDashboardToolbar>
        <template #left>
          <USelectMenu v-model="selectedWorker" icon="i-heroicons-building-office" placeholder="Worker"
            :options="defaultWorkers" :ui-menu="{ option: { base: 'capitalize' } }" />
        </template>
        <template #right>
